#!/usr/bin/env python3
"""Start HavenID api/web outside the Grok Job Object, then exit.

This process is the TUI job. It must die on a hard deadline. The server
must not be in this process's job. Start does not wait for HTTP. Probe is
a separate command.
"""
...
def start_role(role: str, host: str, port: int, timeout: int) -> int:
    ...
    print("Spawned outside the TUI job. Probe with probe-api.ps1 / probe-web.ps1. Do not wait here.")
    return 0
