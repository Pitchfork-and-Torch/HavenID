...
Grok Build wraps each TUI command in a Job Object and waits until that job
is empty. CREATE_BREAKAWAY_FROM_JOB can return success while the child stays
in the job (2026-08-16). A live server left in the job hangs the TUI row
until [stop], which also kills the server.

Rule: never return a pid that is still in a job when this process is in a
job. Prefer parent-spoof (explorer) then WMI then a one-shot scheduled task.
"""
